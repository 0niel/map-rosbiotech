  ...
  return (
    <Highlighter
      highlightClassName={cn(
        props.isActiveOption ? 'bg-yellow-300' : 'bg-yellow-200'
      )}
...
export default function SearchDialog({ open, setOpen }: SearchDialogProps) {
  ...
  const { data: employeeData, isLoading: employeeIsLoading } =
    useQuery<StrapiResponse>('searchEmployees', {
      queryFn: async () => {
      ...
      >
        ...
        >
          ...
          <div className="fixed inset-0 z-30 overflow-y-auto p-2 sm:p-6 md:p-20">
            ...
            >
              <Dialog.Panel className="mx-auto h-full max-w-2xl transform divide-y divide-gray-100 overflow-hidden rounded-xl bg-white shadow-2xl ring-1 ring-black ring-opacity-5 transition-all sm:h-auto sm:w-full sm:max-w-2xl">
                <Combobox>
                  {({ open, activeOption }) => (
                    <>
                      <div className="relative">
                        ...
                        <button
                          type="button"
                          className="absolute right-3 top-3.5 text-gray-400 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 sm:hidden"
                          onClick={() => {
                            closeDialog()
                          }}
                        >
                      ...
                      {query === '' && employeeData === undefined && (
                        <div className="border-t border-gray-100 px-6 py-14 text-center text-sm sm:px-14">
                          ...
                          <p className="mt-4 font-semibold text-gray-900">
                            Поиск сотрудников или аудиторий
                          </p>
                          <p className="mt-2 text-gray-500">
                            Вы можете быстро перейти к нужному аудитории
                          </p>
                        </div>
                      )}
                        ...
                        >
                          ...
                          {popularSearches.map(search => (
                            <Combobox.Option
                              ...
                              className="flex cursor-pointer flex-row items-center space-x-3 p-4 hover:bg-gray-200"
                            >
                              <p className="text-gray-800">{search.query}</p>
                              <p className="text-xs text-gray-500">
                                {search.searchCount} поисков
                              </p>
                            </Combobox.Option>
                          ))}
                        </Combobox.Options>
                      ...
                      {employeeData?.data && employeeData?.data.length > 0 && (
                        <Combobox.Options
                          static
                          className="max-h-full scroll-pb-2 scroll-pt-11 space-y-2 overflow-y-auto pb-2 sm:max-h-80"
                        >
                          <div className="px-4 py-2 text-sm font-medium text-gray-900">
                            Сотрудники
                          </div>
                              ...
                              {({ active }) => (
                                <div className="flex w-full items-center space-x-3">
                                  ...
                                  <div className="flex flex-grow flex-col">
                                    <div className="text-md flex flex-row font-medium text-gray-900">
                                      <SearchHighlighter
                                        textToHighlight={`${
                                      ...
                                      />
                                      {employee.attributes.positions.map(
                                        position => (
                                          <>
                                    ...
                                    <div className="text-xs text-gray-400">
                                      {employee.attributes.positions.map(
                                        position => (
                                          <>
                                            <div
                                              key={`${employee.id}_${position.department}_${position.post}`}
                                            >
                                              {position.post},{' '}
                                              {position.department}
                                            </div>
                        ...
                        >
                          ...
                          {results.map(object =>
                            Object.entries(object).map(([floor, objects]) => (
                              <div key={floor}>
                                <h2 className="p-4 text-lg font-medium text-gray-700">{`Этаж ${floor}`}</h2>
