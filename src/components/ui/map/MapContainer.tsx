...
const MapContainer = () => {
    ...
    const { isLoading, error, data } = useRoomsQuery(campus.shortName, {
        onError: e =>
            toast.error(
                'Ошибка при загрузке информации о кабинетах из расписания'
            ),
        onSuccess: data => {
            setRooms(data)
        },
    ...
    const [selectedRoomOnMap, setSelectedRoomOnMap] =
        useState<RoomOnMap | null>(null)
    ...
    useEffect(() => {
        if (isLoading || !transformComponentRef.current || !mapData || !data)
            return

        const { object } = router.query

        if (object) {
            const mapObject = mapData.getMapObjectById(
                router.query.object as string
            )
            if (!mapObject) {
        ...
        if (start && end) {
            const startMapObject = mapData.getMapObjectById(
                router.query.start as string
            )
            const endMapObject = mapData.getMapObjectById(
                router.query.end as string
            )
        ...
        if (room && campus && date) {
            if (c && (c as string) !== campus.shortName) {
                setCampus(
                    campuses.find(campus => campus.shortName === c) ?? campus
                )
            }
        ...
        (ref: ReactZoomPanPinchRef, event: TouchEvent | MouseEvent) => {
            ...
            if (isPanningRef.current.prevEvent) {
                ...
                if (timeDiff < 200) {
                    ...
                    const elementWithMapObject =
                        element?.closest(mapObjectSelector)

                    const aviableToSelectMapObjectElements =
                        getAllMapObjectsElements(document)

                    const mapObjectElement =
                        aviableToSelectMapObjectElements.find(
                            el => el === elementWithMapObject
                        )

                    if (mapObjectElement) {
                        const isCliclableType =
                            getMapObjectTypeByElemet(mapObjectElement) ===
                            MapObjectType.ROOM
                        if (isCliclableType) {
                ...
                {selectedRoomOnMap && selectedRoomOnMap.mapObject && (
                    <RoomDrawer
                        ...
                        findNearestObject={(
                            ...
                            const mapObject =
                                mapData?.getNearestMapObjectByType(
