    ...
    const getRouteDetailsByPath = useCallback((): DetailsSlide[] => {
        ...
        routeDetails.push({
            ...
            mapObjectToZoom: mapData.getMapObjectById(
                firstPoint.mapObjectId || ''
            ),
            text: 'Начало маршрута',
            icon: (
                <div className="w-2.5 h-2.5 text-blue-800 font-medium rounded-full flex items-center justify-center">
                    А
                </div>
            )
        })
                ...
                if (
                    ...
                    if (nextFloor > currentSegmentFloor) {
                        routeDetails.push({
                            ...
                            icon: (
                                <TbStairsUp className="w-3 h-3 text-blue-800" />
                            )
                        })
                        ...
                        routeDetails.push({
                            ...
                            icon: (
                                <TbStairsDown className="w-3 h-3 text-blue-800" />
                            )
                        })
                    }
                }
                    ...
                    {path &&
                        getRouteDetailsByPath().map((detailsSlide, index) => (
                            ...
                            >
                                ...
                                <p className="flex items-center mb-1 text-base font-normal text-gray-500">
                                    {detailsSlide.text}
                                </p>
                            </li>
                        ))}
