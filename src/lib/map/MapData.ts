...
export class MapData {
                    ...
                    if (
                        ...
                        const neighborId =
                            edge.source === current.id
                                ? edge.target
                                : edge.source
                        const neighbor = graph.vertices.find(
                            v => v.id === neighborId
                        )!
    ...
    getShortestPathBySegments(
        ...
        for (let i = 1; i < path.length; i++) {
            ...
            if (!vert.mapObjectId && !prevVert.mapObjectId) {
                ;(pathsByStairs[pathsByStairs.length - 1] as Vertex[]).push(
                    vert
                )
                continue
            }
            ...
            const prevMapObj = this.getMapObjectById(
                prevVert.mapObjectId as string
            )
    ...
    getNearestMapObjectByType(
        ...
        let mapObjectsWithTargetType =
            this.getAllAvailableObjectsInMap().filter(o => {
        ...
        for (const object of mapObjectsWithTargetType) {
            ...
            for (let i = 0; i < path.length - 1; i++) {
                ...
                const edge = this.unpackedGraph.edges.find(e => {
                    return (
                        (e.source === currentVertex.id &&
                            e.target === nextVertex.id) ||
                        (e.target === currentVertex.id &&
                            e.source === nextVertex.id)
                    )
                })
    ...
    getSearchebleObjects(): SearchableObject[] {
        return this.getAllAvailableObjectsInMap()
            ...
            .map(mapObject => ({
                floor:
                    this.getObjectFloorByMapObjectId(
                        mapObject.id
                    )?.toString() || '',
                mapObject: mapObject
            }))
        ...
        return searchObjects
            .filter(o => {
                if (
                    ...
                    const matchObject =
                        o.mapObject.name.match(roomNumberPattern)
                    ...
                    if (matchObject && matchName) {
                        ...
                        if (numberName) {
                            if (
                                !numberObject
                                    ?.toLowerCase()
                                    .includes(numberName.toLowerCase())
                            ) {
                                return false
                            }
                        }
                        ...
                        if (letterName) {
                            if (
                                letterObject?.toLowerCase() !==
                                letterName.toLowerCase()
                            ) {
                                return false
                            }
                        }
                        ...
                        if (postfixName) {
                            if (
                                postfixObject?.toLowerCase() !==
                                postfixName.toLowerCase()
                            ) {
                                return false
                            }
                        }
